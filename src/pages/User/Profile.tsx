
import Navbar from '../../components/Navbar'
// import { FaUserCircle, FaRegUser } from "react-icons/fa";
import { MdOutlineEmail, MdOutlinePhone, MdOutlineCreditCard } from "react-icons/md";
import { LuUser } from "react-icons/lu";

function Profile() {

    // Yeh data aap bad mein Redux ya API se fetch kar sakte hain
    const profileData = {
        fullName: "John Smith",
        email: "john.smith@yourco.com",
        phone: "+1 234 567 890",
        plan: "Professional Plan",
        price: "$299/month",
    };

    return (
        <div className="bg-[#F9FAFB] min-h-screen font-sans">
            {/* Navbar component */}
            <Navbar />

            <div className="pt-28 pb-10 px-5">
                <div className="max-w-[1000px] mx-auto space-y-8">

                    {/* Header Section: Avatar & Basic Info */}
                    <div className="flex items-center gap-4 mb-10">
                        <div className="text-white w-16 h-16 rounded-full flex items-center justify-center"
                            style={{
                                background: "linear-gradient(135deg, #2B7FFF 0%, #4F39F6 100%)"
                            }}
                        >
                            <LuUser size={32} />
                        </div>
                        <div>
                            <h1 className="text-2xl md:text-3xl font-bold text-[#111827]">
                                {profileData.fullName}
                            </h1>
                            <p className="text-gray-500 text-sm md:text-base">
                                {profileData.email}
                            </p>
                        </div>
                    </div>

                    {/* Section 1: Profile Information Card */}
                    <div className="bg-white border border-gray-200 rounded-xl p-6 md:p-8">
                        <div className="flex items-center gap-2 mb-1 border-b border-transparent">
                            <LuUser className="text-gray-700 text-xl" />
                            <h2 className="text-xl font-medium text-[#111827]">Profile Information</h2>
                        </div>
                        <p className="text-sm text-gray-500 mb-6">Your personal information and contact details</p>

                        <div className="space-y-5">
                            {/* Full Name Field */}
                            <div className="space-y-1.5">
                                <label className="flex items-center gap-2 text-xs font-medium text-gray-500 uppercase tracking-wider">
                                    <LuUser size={16} /> Full Name
                                </label>
                                <input
                                    type="text"
                                    readOnly
                                    value={profileData.fullName}
                                    className="w-full bg-[#F9FAFB] border border-gray-200 rounded-lg px-4 py-3 text-gray-700 outline-none"
                                />
                            </div>

                            {/* Email Field */}
                            <div className="space-y-1.5">
                                <label className="flex items-center gap-2 text-xs font-medium text-gray-500 uppercase tracking-wider">
                                    <MdOutlineEmail size={16} /> Email Address
                                </label>
                                <input
                                    type="email"
                                    readOnly
                                    value={profileData.email}
                                    className="w-full bg-[#F9FAFB] border border-gray-200 rounded-lg px-4 py-3 text-gray-700 outline-none"
                                />
                            </div>

                            {/* Phone Field */}
                            <div className="space-y-1.5">
                                <label className="flex items-center gap-2 text-xs font-medium text-gray-500 uppercase tracking-wider">
                                    <MdOutlinePhone size={16} /> Phone Number
                                </label>
                                <input
                                    type="text"
                                    readOnly
                                    value={profileData.phone}
                                    className="w-full bg-[#F9FAFB] border border-gray-200 rounded-lg px-4 py-3 text-gray-700 outline-none"
                                />
                            </div>
                        </div>
                    </div>

                    {/* Section 2: Active Subscription Plan Card */}
                    <div className="bg-white border border-gray-200 rounded-xl p-6 md:p-8">
                        <div className="flex items-center gap-2 mb-1">
                            <MdOutlineCreditCard className="text-gray-700 text-xl" />
                            <h2 className="text-lg font-semibold text-[#111827]">Active Subscription Plan</h2>
                        </div>
                        <p className="text-sm text-gray-500 mb-6">Your current plan details</p>

                        <div className="bg-[#eff6ff] border border-blue-100 rounded-xl p-6">
                            <div className="flex items-center gap-3 mb-2">
                                <span className="font-bold text-[#111827] text-lg">{profileData.plan}</span>
                                <span className="bg-[#dcfce7] text-[#15803d] text-[10px] font-bold px-2 py-0.5 rounded-full uppercase">
                                    Active
                                </span>
                            </div>
                            <div className="text-[#2563eb] text-2xl md:text-3xl font-bold">
                                {profileData.price}
                            </div>
                        </div>
                    </div>

                </div>
            </div>
        </div>
    )
}

export default Profile
